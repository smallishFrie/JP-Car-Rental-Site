...
function CarCardContent({ car, formatDayRate }: { car: Car; formatDayRate: (rate: number) => string }) {
  return (
    <>
      <div className="car-card-image-wrap">
        <Image src={car.cardImage} alt="" width={1280} height={720} className="car-card-image" />
      </div>
...
export default function CarsBrowser({ cars }: CarsBrowserProps) {
    ...
    return cars
      .filter((car) => {
        ...
        const categoryMatch =
          ...
          selectedCategories.some((selectedCategory) => carCategories.includes(selectedCategory));
      ...
      <div className="cars-grid-shell">
        ...
        <div className="cars-grid">
          {filteredCars.map((car) => (
            <Link key={car.id} href={`/cars/${car.id}`} className="car-card" aria-labelledby={`car-card-title-${car.id}`}>
  ...
  return (
    <KineticStagger className="cars-grid-shell">
      <header className="cars-grid-header" id="available-cars-header">
        <KineticItem as="motion.div" preset={homePresetAt(HOME_FLEET_INTRO_PRESETS, 0)}>
          <h3>Available Cars</h3>
          <p>Choose your ride and continue to booking details.</p>
        </KineticItem>
      </header>
      ...
      >
        ...
        <KineticItem as="motion.div" preset={homePresetAt(HOME_FLEET_INTRO_PRESETS, 2)}>
          {categoryFilter}
        </KineticItem>
      </motion.div>

      <motion.div className="cars-grid" variants={kineticFleetStaggerContainer}>
        {filteredCars.map((car, index) => (
          <KineticItem key={car.id} as="motion.div" className="car-card-tilt-wrapper" preset={homeFleetCardPreset(index)}>
